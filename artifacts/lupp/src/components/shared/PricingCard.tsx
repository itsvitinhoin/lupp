...
export function PricingCard({ name, price, features, isPopular, ctaText = 'Assinar agora', onSelect, selected }: PricingCardProps) {
  return (
    <Card className={`relative overflow-hidden transition-all ${isPopular ? 'border-primary shadow-lg shadow-primary/20' : 'border-white/5'} ${selected ? 'ring-2 ring-primary' : ''}`}>
      {isPopular && (
