...
(() => {
  ...
  (function() {
    ...
    function getNuvemshopCartBridge() {
      return window.__LUUP_NUVEMSHOP_ADD_TO_CART__ || window.LuupNuvemshopCart && window.LuupNuvemshopCart.addItems;
    }
    function waitForNuvemshopCartBridge(deadline) {
      var bridge2 = getNuvemshopCartBridge();
      if (typeof bridge2 === "function") return Promise.resolve(bridge2);
    ...
    function addNuvemshopItemsToCart(items) {
      ...
      return waitForNuvemshopCartBridge(Date.now() + 6e3).then(function(bridge2) {
        return Promise.resolve(bridge2(validItems)).then(function(result) {
          notifyNuvemshopCartUpdated(validItems);
          return result || {};
        });
      }).catch(function(bridgeError) {
        return addNuvemshopItemsWithNativeCart(validItems).then(function(result) {
          notifyNuvemshopCartUpdated(validItems);
          return result || {};
        }).catch(function() {
          throw bridgeError;
    ...
    window.addEventListener("message", function(event) {
      var data = event.data || {};
      if (!data || data.type !== "LUPP_NUVEMSHOP_ADD_TO_CART_REQUEST" || !isTrustedLuppFrameOrigin(event.origin)) {
        return;
      }
      addNuvemshopItemsToCart(data.items).then(function() {
        postFrameResponse(
          event.source,
          event.origin,
        ...
        postFrameResponse(
          ...
          data.requestId,
          {
            error: error && error.message ? error.message : "nuvemshop_cart_request_failed",
            ok: false
          }
        );
      });
