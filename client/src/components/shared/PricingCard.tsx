...
export function PricingCard({ name, price, features, isPopular, ctaText = 'Assinar agora', onSelect, selected }: PricingCardProps) {
  return (
    <Card className={`relative overflow-hidden bg-white transition-all ${isPopular ? 'border-primary shadow-lg shadow-primary/20' : ''} ${selected ? 'ring-2 ring-primary' : ''}`}>
      {isPopular && (
      ...
      <CardHeader>
        ...
        <div className="mt-2 flex items-baseline gap-1">
          <span className="text-3xl font-bold text-slate-950">R${price}</span>
